...
export class Keymap<T extends KeymapConfig = KeymapConfig> extends JWPlugin<T> {
     ...
     * @param command
     */
    bind(pattern: string, command: ConfiguredCommand, level: LEVEL = LEVEL.DEFAULT): void {
    ...
    listener(event: KeyboardEvent): void {
        const commands = this.match(event);
        ...
        if (command && command.commandId) {
            ...
            this.editor.execCommand(command.commandId, params);
        }
    }
    ...
    _loadShortcuts(shortcuts: Shortcut[], source?: JWPlugin | JWEditorConfig): void {
        for (const shortcut of shortcuts) {
            if (!shortcut.platform || shortcut.platform === this.config.platform) {
                if (!shortcut.platform && this.config.platform === Platform.MAC) {
                    shortcut.pattern = shortcut.pattern.replace(/ctrl/gi, 'CMD');
                }
                if (source instanceof JWPlugin) {
                    this.bind(shortcut.pattern, shortcut, LEVEL.DEFAULT);
                } else {
                    this.bind(shortcut.pattern, shortcut, LEVEL.USER);
                }
            }
        }
    }
