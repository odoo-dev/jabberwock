...
export class FontSizeDomObjectRenderer extends InputDomObjectRenderer {
     ...
     * @override
     */
    _onEditorChange(node: InputNode, input: HTMLInputElement): void {
        super._onEditorChange(node, input);
        const range = this.engine.editor.selection.range;
        const fontNodes = range.selectedNodes(CharNode);

        const fontSize =
            fontNodes[0] && fontNodes[0].modifiers.get(Attributes).style.get('font-size');
        ...
        if (fontSize) {
            ...
            const style = getComputedStyle(firstDomElement);
