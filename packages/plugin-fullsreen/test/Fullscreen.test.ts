...
describe('Fullscreen', () => {
    ...
    it('sould start the fullscreen mode', async () => {
        ...
        expect(container.querySelector('button[name="fullscreen"]').outerHTML).to.equal(
            '<button name="fullscreen" class="fas fa-expand fa-fw pressed" title="Toggle Fullsreen" aria-pressed="true"></button>',
        );
        await editor.stop();
    ...
    it('sould stop the fullscreen mode', async () => {
        ...
        expect(document.body.classList.contains('jw-fullscreen')).to.equal(false, 'body');
        expect(container.querySelector('button[name="fullscreen"]').outerHTML).to.equal(
            '<button name="fullscreen" class="fas fa-expand fa-fw" title="Toggle Fullsreen" aria-pressed="false"></button>',
        );
        await editor.stop();
    });
