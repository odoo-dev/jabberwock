...
interface BackgroundColorConfig extends JWPluginConfig {
...
export class BackgroundColor<T extends BackgroundColorConfig = BackgroundColorConfig> extends JWPlugin<T> {
    static dependencies = [Inline];
    configuration = { ...defaultConfiguration, ...this.configuration };
    commands = {
        colorBackground: {
            handler: this.colorBackground,
        },
        uncolorBackground: {
            handler: this.uncolorBackground,
        },
    };
    ...
    static hasColor(color: string | VNode, node?: VNode): boolean {
        ...
        const nodeBackgroundColor = getStyle(node, 'background-color');
        if (color instanceof VNode) {
    ...
    /**
     * Apply the given background color to the range.
     *
     * @param params
     */
    colorBackground(params: ColorParams): void {
        ...
        if (params.context.range.isCollapsed()) {
            ...
            for (const node of this._highestSelectedNodes(params.context.range)) {
                ...
                const colorAncestor = node.ancestor(BackgroundColor.hasColor);
                if (colorAncestor && BackgroundColor.hasColor(color, colorAncestor)) continue;
    ...
    /**
     * Remove the current background color from the range. If the color was
     * applied to an ancestor, apply the default color to its relevant inline
     * descendants.
     *
     * @param params
     */
    uncolorBackground(params: CommandParams): void {
        const range = params.context.range;
        const defaultColor = this.configuration.defaultBackgroundColor;
        const hasColor = BackgroundColor.hasColor;
        const styleName = 'background-color';
        ...
        if (range.isCollapsed()) {
            ...
            if (range.start.ancestor(hasColor)) {
                // Set the background color style cache to the default color.
                const currentCache = inline.cache.style || {};
                inline.cache.style = { ...currentCache, [styleName]: defaultColor };
            } else if (inline.cache.style) {
                // Unset the background color style cache.
    ...
    _isAllColored(node: VNode, color: string): boolean {
        return node.children().every(child => BackgroundColor.hasColor(color, child));
    }
}
