...
import {
    makeVersionable,
    markNotVersionable,
    createProxyWithVersionable,
} from '../../src/Memory/Versionable';
        ...
        describe('memory', () => {
            ...
            describe('snapshot', () => {
                ...
                it('snapshot array changes', () => {
                    ...
                    const ID = Object.keys(memory._slices.test)[0];
                    let patch = memory._slices.test[ID];
                    expect(Object.values(patch.patch)).to.deep.equal([1]);

                    memory.snapshot('test', 'test-1', 'snap');

                    expect(Object.keys(memory._slices.snap).length).to.equal(1);
                    patch = memory._slices.snap[ID];
                ...
                it('snapshot object changes', () => {
                    ...
                    expect(JSON.stringify(patch)).to.equal('{"a-z":' + ID1 + ',"x+y":2}');
                    ...
                    expect(JSON.stringify(patch)).to.equal('{"a-z":' + ID2 + ',"x+y":1,"a+b":3}');
                });
                it('auto-snapshot', () => {
                    const memory = new Memory();
                    memory._numberOfFlatSlices = 20;
                ...
                it('compress array changes', () => {
                    ...
                    expect(Object.keys(memory._slices.test).length).to.equal(2);
                    patch = memory._slices.test[ID];
                    expect(Object.values(patch.patch)).to.deep.equal([1, 2, 3]);
                    patch = memory._slices.test[obj[memoryProxyPramsKey].ID];
                    expect(patch).to.deep.equal({ 'x+y': 3 });
                });
            });
        });
    });
