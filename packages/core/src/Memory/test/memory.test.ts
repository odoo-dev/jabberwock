        ...
        describe('memory', () => {
            describe('throw', () => {
                ...
                it('if try to link a non versionable object', () => {
                    ...
                    expect((): void => {
                        memory.linkToMemory([1]);
                    }).to.throw('VersionableObject', 'array');
                    expect((): void => {
                        memory.linkToMemory(new Set([1]));
                ...
                it('if try to link a versionable object to 2 memories', () => {
                    ...
                    memoryTest.linkToMemory(obj);
                    const array = new VersionableArray();
                    memoryTest.linkToMemory(array);
                    const set = new VersionableSet();
                    memoryTest.linkToMemory(set);
            ...
            describe('create versionable', () => {
                ...
                it('custom class who contains proxy of Versionable', () => {
                    class SuperObject extends VersionableObject {
                        ...
                        constructor() {
                            super();
                            this.obj = createProxyWithVersionable(new VersionableObject(), {
            ...
            describe('specific updates', () => {
                ...
                it('array push and pop in same slide memory have a clean memory slice values', () => {
                    ...
                    array.pop();
                    array.pop();
                    const currentSlice = memory._currentReference.slice;
            ...
            describe('memory slices', () => {
                ...
                it('remove slice avoid memory leak', () => {
                    ...
                    memory.linkToMemory(array1);

                    array1.push(obj);

                    memory.switchTo('2');
                    const array2 = new VersionableArray();
                    memory.linkToMemory(array2);
            ...
            describe('snapshot', () => {
                it('create a snapshot from 8 slices', () => {
                    ...
                    memory.create('other');
                    memory.switchTo('other');
                    expect(memory.path('other').join()).to.deep.equal(
                    ...
                    expect(memory.path('after-snap').join()).to.deep.equal(
                        'after-snap,snap,test,1',
                    );
                    expect(memory.path('after-snap', true).join()).to.deep.equal(
                        'after-snap,snap,test-6,test-5,test-4,test-3,test-2,test-1,test-0,test,1',
                    );
                    expect(memory.path('other').join()).to.deep.equal('other,snap,test,1');
                    expect(memory.path('test-7').join()).to.deep.equal(
                ...
                it('snapshot array changes', () => {
                    ...
                    const ID = Object.keys(memory._references.test.slice)[0];
                    let patch = memory._references.test.slice[ID];
                    expect(Object.values(patch.patch)).to.deep.equal([1]);

                    memory.snapshot('test', 'test-1', 'snap');

                    expect(Object.keys(memory._references.snap.slice).length).to.equal(1);
                    patch = memory._references.snap.slice[ID];
                ...
                it('auto-snapshot', () => {
                    ...
                    const sliceKeys = Object.keys(memory._references);
                ...
                it('compress array changes', () => {
                    ...
                    let patch = memory._references.test.slice[ID];
                    expect(Object.values(patch.patch)).to.deep.equal([1]);
                    expect(Object.keys(memory._references).length).to.equal(6);

                    memory.compress('test', 'test-1');

                    expect(Object.keys(memory._references).length).to.equal(4);
                    expect(Object.keys(memory._references.test.slice).length).to.equal(2);
                    patch = memory._references.test.slice[ID];
                    expect(Object.values(patch.patch)).to.deep.equal([1, 2, 3]);
                    patch = memory._references.test.slice[obj[memoryProxyPramsKey].ID];
            ...
            describe('root & path', () => {
                it('link to memory & markAsDiffRoot', () => {
                    ...
                    memory.switchTo('test');
                    memory.linkToMemory(obj);
                    expect(memory.getRootVersionables(obj.toto.titi)).to.deep.equal(
                        new Set([obj.toto]),
                        'titi',
                    );
                    expect(memory.getRootVersionables(obj.toto)).to.deep.equal(
                        new Set([obj.toto]),
                        'toto',
                    );
                    expect(memory.getRootVersionables(obj.tutu)).to.deep.equal(
                        new Set([obj]),
                        'tutu',
                    );
                });
                it('link to memory & markAsDiffRoot with change slice', () => {
                    ...
                    const test1 = [];
                    memory.getRootVersionables(obj.tutu).forEach(obj => test1.push(obj));
                    expect(test1).to.deep.equal([obj.toto, obj], 'tutu');
                    const test2 = [];
                    memory.getRootVersionables(obj.toto.tata).forEach(obj => test2.push(obj));
