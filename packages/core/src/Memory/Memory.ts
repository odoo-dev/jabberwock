...
export class Memory {
    ...
    getRootVersionables(proxy: LoopObject | LoopArray | LoopSet): Set<AllowedObject> {
        ...
        let nodeID = proxy[memoryProxyPramsKey].ID as ProxyUniqID;
        ...
        while ((nodeID = nodeIDs.pop())) {
            ...
            this._getProxyParentedPath(this._sliceKey, nodeID).forEach(path => {
                nodeIDs.push(+path.split(parentedPathSeparator, 1));
            });
        }
        return roots;
    }
    ...
    _addSliceProxyParent(ID: ProxyUniqID, parentID: ProxyUniqID, attributeName: string): void {
    ...
    _deleteSliceProxyParent(ID: ProxyUniqID, parentID: ProxyUniqID, attributeName: string): void {
        const sliceKey = this._sliceKey;
        const sliceLinkedParentOfProxy = this._references[sliceKey].linkedParentOfProxy;
        const path =
            parentID + (attributeName === undefined ? '' : parentedPathSeparator + attributeName);
    ...
    _getProxyParentedPath(sliceKey: sliceKey, ID: ProxyUniqID): proxyAttributePath {
    ...
    _getValue(sliceKey: string, ID: ProxyUniqID): MemoryCompiled {
    ...
    _getValueArray(sliceKey: string, ID: ProxyUniqID, patches: patches): MemoryCompiled {
