...
import { withDomMutation } from '../src/mutationTransaction';
...
async function load(
    params: { content?: string | HTMLElement; debug?: boolean },
    callback: (ctx: {
        editor: BasicEditor;
...
describe('mutationTransaction', () => {
    describe('attributes', () => {
        it('should add attribute', async () => {
            await load({ content: '<div>xx</div>' }, async ctx => {
                ...
                await withDomMutation(ctx.editor, ctx.editableNode, () => {
                ...
                describe('add link', () => {
                    ...
                    it('should add <a>ab</a> then <b>cd</b> in <div></div>', async () => {
                        await load({ content: '<div></div>' }, async ctx => {
                            ...
                            b.textContent = 'cd';
                            await withDomMutation(ctx.editor, ctx.editableNode, async context => {
                                div.appendChild(a);
                                div.appendChild(b);
                            });
    ...
    describe('characterData', () => {
        ...
        // it('should add a space to character data if last inline is not a space', async () => {
        //     const div = document.createElement('div');
        //     div.setAttribute('id', 'main');
        //     div.innerHTML = '<span>a</span>';
        //     document.body.append(div);

        //     const editor = new BasicEditor({ editable: div });
        //     editor.load(DomHelpers);

        //     await mutationTransaction(editor, div, () => {
        //         const span = document.querySelector('span');
        ...
        //     const editable = domEngine.components.editable[0];
        //     expect(editable.childVNodes[0]).to.be.instanceOf(CharNode);
        //     expect((editable.childVNodes[0] as CharNode).char).to.be('a');
        //     expect(editable.childVNodes[1]).to.be.instanceOf(CharNode);
        //     expect((editable.childVNodes[1] as CharNode).char).to.be(' ');
        //     expect(editable.childVNodes[2]).to.be.instanceOf(CharNode);
        //     expect((editable.childVNodes[2] as CharNode).char).to.be('b');
    ...
    it('should return what the callback return', async () => {
        await load({ content: '<div></div>' }, async ctx => {
            const result = await withDomMutation(ctx.editor, ctx.editableNode, () => {
                return 1;
            });
    ...
    // it('should compute the mininum when mutation is inside mutation', async () => {
    //     await load({ content: '<div></div>' }, async ctx => {
    //         const div = document.querySelector('div');
    //         const a = document.createElement('a');
    //         a.textContent = 'ab';
    //         await withDomMutation(ctx.editor, ctx.editableNode, async span => {
    //             div.appendChild(a);
    //             await withDomMutation(ctx.editor, ctx.editableNode, () => {
    //                 div.appendChild(a);
    //             });
    ...
    describe('withDomMutation inside withDomMutation', () => {
        it('should only add once when a mutation is inside another mutation', async () => {
            await load({ content: '<div><span>ab</span></div>' }, async ctx => {
                ...
                const a = document.createElement('a');
                a.textContent = 'cd';
                await withDomMutation(ctx.editor, span, async context => {
                    await withDomMutation(
                        ctx.editor,
                        div,
                        () => {
                            span.appendChild(a);
        ...
        it('should discard the need to observe if a node is already included', async () => {
            await load({ content: '<div><span>ab</span></div>' }, async ctx => {
                ...
                const a = document.createElement('a');
                a.textContent = 'cd';
                await withDomMutation(ctx.editor, div, async context => {
                    await withDomMutation(
                        ctx.editor,
                        span,
                        () => {
    ...
    it('should add a node when observed within an iframe', async () => {});
});
