...
import { RangeDirection, VRange } from '../stores/VRange';
...
export class TestEnvironment {
    ...
    constructor(template = '', debug = false) {
        ...
        this.el = this.editor.el as Node;
        this.root = this.editor.vDocument.root;
     ...
     */
    testDOMRange(range: RangeDescription): void {
    ...
    testVRange(paths: RangePath, backward = false): void {
        ...
        const hasRangeCommonParent = this.range._start.parent.children.some(
            child => child.id === this.range._end.id,
        );
        ...
        expect(this.range[backward ? '_end' : '_start'].id).to.equal(start.id);
        expect(this.range[backward ? '_start' : '_end'].id).to.equal(end.id);
        expect(this.range.direction).to.equal(
            backward ? RangeDirection.BACKWARD : RangeDirection.FORWARD,
        );
    }
     ...
     */
    async setRange(range: RangeDescription, backward = false): Promise<void> {
     ...
     */
    async triggerNativeEvents(target: Node, events: string | string[]): Promise<void> {
        const options = {
            view: window,
            bubbles: true,
            cancelable: true,
    ...
    _pathToExpectedRange(path: number[], offset: number): number[] {
        ...
        path.forEach((index, i, self) => {
            container = container.childNodes[index];
            if (container && ['B', 'I', 'U'].includes(container.nodeName)) {
                self.splice(i, 1);
                if (index) {
     ...
     *
     * @private
     * @param {RangeDescription} range
     * @param {boolean} [backward]
     */
    _selectRange(range: RangeDescription, backward = false): void {
        range.endContainer = range.endContainer || range.startContainer;
        range.endOffset = typeof range.endOffset === 'number' ? range.endOffset : range.startOffset;
        if (backward) {
