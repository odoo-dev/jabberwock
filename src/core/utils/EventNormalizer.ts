...
export class EventNormalizer {
    ...
    _triggerEvent: (customEvent: CustomEvent) => void; // callback to trigger on events
    _rangeHasChanged: boolean;

    constructor(editable: HTMLElement, triggerEvent: (customEvent: CustomEvent) => void) {
    ...
    _processCompiledEvent(): void {
        ...
        if (compiledEvent.defaultPrevented) {
            ...
            this._formatAndTriggerEvent('inconsistentState');
            return;
        }
        ...
        if (navigationKey.has(compiledEvent.key)) {
            ...
            this._formatAndTriggerEvent('remove', deleteEventPayload, elements);
            ...
            this._formatAndTriggerEvent('tab', tabEventPayload, elements);
            ...
            this._formatAndTriggerEvent('enter', enterEventPayload, elements);
        } else if (
            ...
            this._formatAndTriggerEvent('keydown', compiledEvent);
        } else {
            ...
            this._formatAndTriggerEvent('inconsistentState', compiledEvent);
        }
    }
    /**
     * Return a properly formatted CustomEvent, based on the given type
     * and optional params.
     *
     * @param {string} type
     * @param {object} [params]
     ...
     * @returns {CustomEvent}
     */
    _formatCustomEvent(type: string, params = {}, elements?: Set<HTMLElement>): CustomEvent {
        const initDict = {
            detail: params,
        };
        initDict.detail['origin'] = 'EventNormalizer';
        if (elements) {
            initDict.detail['elements'] = elements;
        }
     ...
     * @param {Set<HTMLElement>} [elements]
     */
    _formatAndTriggerEvent(type: string, params = {}, elements?: Set<HTMLElement>): void {
        this._triggerEvent(this._formatCustomEvent(type, params, elements));
    }
    /**
    ...
    _processComposition(ev: CompiledEvent, elements: Set<HTMLElement>): void {
        if (!this.editable.contains(ev.clone.origin)) {
            ...
            this._formatAndTriggerEvent('inconsistentState', ev, elements);
            return;
        }
    ...
    _processMove(ev: CompiledEvent, elements: Set<HTMLElement>): void {
        ...
        if (ev.defaultPrevented) {
            this._formatAndTriggerEvent('restoreRange');
        } else {
    ...
    _onSelectionChange(): void {
        ...
        if (
            ...
            this._triggerEvent(
                this._formatCustomEvent('selectAll', {
